import numpy as np


class KNN:
    """
    K Nearest Neighbours model
    Args:
        k_neigh: Number of neighbours to take for prediction
        weighted: Boolean flag to indicate if the nieghbours contribution
                  is weighted as an inverse of the distance measure
        p: Parameter of Minkowski distance
    """

    def __init__(self, k_neigh, weighted=False, p=2):

        self.weighted = weighted
        self.k_neigh = k_neigh
        self.p = p

    def fit(self, data, target):
        """
        Fit the model to the training dataset.
        Args:
            data: M x D Matrix( M data points with D attributes each)(float)
            target: Vector of length M (Target class for all the data points as int)
        Returns:
            The object itself
        """

        self.data = data
        self.target = target.astype(np.int64)

        return self

    def find_distance(self, x):
        """
        Find the Minkowski distance to all the points in the train dataset x
        Args:
            x: N x D Matrix (N inputs with D attributes each)(float)
        Returns:
            Distance between each input to every data point in the train dataset
            (N x M) Matrix (N Number of inputs, M number of samples in the train dataset)(float)
        """
        list_of_dist = []
        for data_point in x:
            list_of_pnts = []
            for pnt1 in self.data:
                value_of_diff = abs(data_point - pnt1)
                value_of_diff = value_of_diff ** self.p
                mink_dist = 0
                for val in value_of_diff:
                    mink_dist = mink_dist + val
                mink_dist = mink_dist ** (1 / self.p)
                list_of_pnts.append(mink_dist)
            list_of_dist.append(list_of_pnts)
        return list_of_dist

    def k_neighbours(self, x):
        """
        Find K nearest neighbours of each point in train dataset x
        Note that the point itself is not to be included in the set of k Nearest Neighbours
        Args:
            x: N x D Matrix( N inputs with D attributes each)(float)
        Returns:
            k nearest neighbours as a list of (neigh_dists, idx_of_neigh)
            neigh_dists -> N x k Matrix(float) - Dist of all input points to its k closest neighbours.
            idx_of_neigh -> N x k Matrix(int) - The (row index in the dataset) of the k closest neighbours of each input

            Note that each row of both neigh_dists and idx_of_neigh must be SORTED in increasing order of distance
        """
        # TODO
        list_of_dist = []
        for data_point in x:
            list_of_pnts = []
            for pnt1 in self.data:
                value_of_diff = abs(data_point - pnt1)
                value_of_diff = value_of_diff ** self.p
                mink_dist = 0
                for val in value_of_diff:
                    mink_dist = mink_dist + val
                mink_dist = mink_dist ** (1 / self.p)
                list_of_pnts.append(mink_dist)
            list_of_dist.append(list_of_pnts)
        point_dist = list_of_dist
        neigh_dist= []
        neigh_index = []
        for pnt in point_dist:
            list = []
            index = 0
            for a in pnt:
                list.append([a,index])
                index = index + 1
            list = sorted(list,key=lambda x:x[0])
            dist = []
            idx = []
            for m in range(0,self.k_neigh):
                dist.append(list[m][0])
                idx.append(list[m][1])
            neigh_dist.append(dist)
            neigh_index.append(idx)
        return neigh_dist,neigh_index
        pass

    def predict(self, x):
        """
        Predict the target value of the inputs.
        Args:
            x: N x D Matrix( N inputs with D attributes each)(float)
        Returns:
            pred: Vector of length N (Predicted target value for each input)(int)
        """
        # TODO
        list_of_dist = []
        for data_point in x:
            list_of_pnts = []
            for pnt1 in self.data:
                value_of_diff = abs(data_point - pnt1)
                value_of_diff = value_of_diff ** self.p
                mink_dist = 0
                for val in value_of_diff:
                    mink_dist = mink_dist + val
                mink_dist = mink_dist ** (1 / self.p)
                list_of_pnts.append(mink_dist)
            list_of_dist.append(list_of_pnts)
        point_dist = list_of_dist    
        neigh_dist= []
        neigh_index = []
        for pnt in point_dist:
            list = []
            index = 0
            for a in pnt:
                list.append([a,index])
                index = index + 1
            list = sorted(list,key=lambda x:x[0])
            dist = []
            idx = []
            for m in range(0,self.k_neigh):
                dist.append(list[m][0])
                idx.append(list[m][1])
            neigh_dist.append(dist)
            neigh_index.append(idx)
        neigh_dist,neigh_index = self.k_neighbours(x)
        pred = []
        eps = 1e-9
        for w in range(0,len(neigh_index)):
            p = dict()
            indx = neigh_index[w]
            for ind in range(0,len(indx)):
                if(self.weighted):
                    if(self.target[indx[ind]] not in p):
                        p[self.target[indx[ind]]] = 1/(neigh_dist[w][ind] + eps)
                    else:
                        p[self.target[indx[ind]]]+= 1/(neigh_dist[w][ind] + eps)
                else:
                    if(self.target[indx[ind]] not in p):
                        p[self.target[indx[ind]]] = 1
                    else:
                        p[self.target[indx[ind]]] = p[self.target[indx[ind]]] + 1
            mvalue = max(p, key = lambda x: p[x])
            pred.append(mvalue)
        return pred             


        

    def evaluate(self, x, y):
        """
        Evaluate Model on test data using 
            classification: accuracy metric
        Args:
            x: Test data (N x D) matrix(float)
            y: True target of test data(int)
        Returns:
            accuracy : (float.)
        """
        # TODO
        list_of_dist = []
        for data_point in x:
            list_of_pnts = []
            for pnt1 in self.data:
                value_of_diff = abs(data_point - pnt1)
                value_of_diff = value_of_diff ** self.p
                mink_dist = 0
                for val in value_of_diff:
                    mink_dist = mink_dist + val
                mink_dist = mink_dist ** (1 / self.p)
                list_of_pnts.append(mink_dist)
            list_of_dist.append(list_of_pnts)
        point_dist = list_of_dist    
        neigh_dist= []
        neigh_index = []
        for pnt in point_dist:
            list = []
            index = 0
            for a in pnt:
                list.append([a,index])
                index = index + 1
            list = sorted(list,key=lambda x:x[0])
            dist = []
            idx = []
            for m in range(0,self.k_neigh):
                dist.append(list[m][0])
                idx.append(list[m][1])
            neigh_dist.append(dist)
            neigh_index.append(idx)
        neigh_dist,neigh_index = self.k_neighbours(x)
        pred = []
        eps = 1e-9
        for w in range(0,len(neigh_index)):
            p = dict()
            indx = neigh_index[w]
            for ind in range(0,len(indx)):
                if(self.weighted):
                    if(self.target[indx[ind]] not in p):
                        p[self.target[indx[ind]]] = 1/(neigh_dist[w][ind] + eps)
                    else:
                        p[self.target[indx[ind]]]+= 1/(neigh_dist[w][ind] + eps)
                else:
                    if(self.target[indx[ind]] not in p):
                        p[self.target[indx[ind]]] = 1
                    else:
                        p[self.target[indx[ind]]] = p[self.target[indx[ind]]] + 1
            mvalue = max(p, key = lambda x: p[x])
            pred.append(mvalue)
        user_predicted_value = pred
        value_of_count = 0
        for z in range(0,len(y)):
            if(user_predicted_value[z] == y[z]):
                value_of_count = value_of_count + 1
        value_of_accuracy = ( value_of_count / len(y))*100
        return value_of_accuracy
        pass
